...
fn main() {
    ...
    let runs = bench_runs();
    let lock = hotpath::rw_lock!(std::sync::RwLock::new(0u64), label = "counter");
    ...
    println!(
        "std RwLock writes: {runs} in {write_elapsed:?} ({:.1} ns/op)",
        write_elapsed.as_nanos() as f64 / runs as f64
    );
    println!(
        "std RwLock reads:  {runs} in {read_elapsed:?} ({:.1} ns/op)",
        read_elapsed.as_nanos() as f64 / runs as f64
    );
    println!("Final value: {}, read acc: {acc}", *lock.read().unwrap());
}
