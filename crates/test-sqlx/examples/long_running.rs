//! Long-running variant that keeps issuing queries so the metrics server stays
//! up for live TUI / endpoint inspection.
//!
//! Terminal 1:
//!   cargo run -p test-sqlx --example long_running --features hotpath
//! Terminal 2:
//!   cargo run --bin hotpath --features tui -- console --metrics-port 6770
//!   # then press [2] for Data Flow and cycle to the SQL sub-tab
//!
