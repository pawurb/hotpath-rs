...
//! overrides it.
//!
//! `HOTPATH_META_UPLOAD_PATH` writes the payload to a file instead of posting
//! it, for a job that gets no OIDC token because it runs untrusted code: a fork
...
/// Path the report is written to, from `HOTPATH_META_UPLOAD_PATH`. Relative paths
