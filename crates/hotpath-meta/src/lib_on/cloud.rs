...
//! with the job's OIDC token. Enabled at runtime by `HOTPATH_META_UPLOAD=1`;
//! the benchmark name comes from `HOTPATH_META_BENCHMARK` (default `default`,
//! validated by `validate_benchmark_name` - invalid names skip the upload).
//! The target base URL is `https://hotpath.rs` unless `HOTPATH_META_UPLOAD_URL`
//! overrides it.
//!
//! `HOTPATH_META_UPLOAD_PATH` writes the same payload to a file instead of - or
//! as well as - posting it. That is the path for a job which measures untrusted
...
/// Base URL the report is posted to. `HOTPATH_META_UPLOAD_URL` overrides it
/// for staging or self-hosted backends; trailing slashes are trimmed so the
...
/// Path the report is written to, from `HOTPATH_META_UPLOAD_PATH`. Relative
/// paths resolve against the working directory, blank or unset writes nothing.
pub(crate) static UPLOAD_PATH: LazyLock<Option<PathBuf>> =
    LazyLock::new(|| parse_upload_path(std::env::var("HOTPATH_META_UPLOAD_PATH").ok()));
...
/// cannot be undone. Replaces `HOTPATH_META_LIMIT`, every
/// `HOTPATH_META_<SECTION>_LIMIT` and the builder limits for the upload only;
/// unparsable values fall back to `0`.
...
pub(crate) fn write_payload(path: &Path, report: &JsonReport) {
    let benchmark = match benchmark_name() {
        Ok(name) => name,
        Err(msg) => {
            eprintln!("hotpath-meta: upload payload not written: {msg}");
            return;
        }
    };
    ...
    match result {
        Ok(()) => eprintln!(
            "hotpath-meta: wrote upload payload to {} (benchmark {benchmark})",
            path.display()
        ),
        Err(reason) => eprintln!("hotpath-meta: upload payload not written: {reason}"),
    }
}
...
pub(crate) fn upload(report: &JsonReport) {
    ...
    let (request_url, request_token) = match (
        ...
        _ => {
            eprintln!(
                "hotpath-meta: upload skipped: not in GitHub Actions or missing `id-token: write` permission"
            );
            return;
        }
    };
    ...
    let benchmark = match benchmark_name() {
        Ok(name) => name,
        Err(msg) => {
            eprintln!("hotpath-meta: upload skipped: {msg}");
            return;
        }
    };
    ...
    match result {
        Ok(resp) => eprintln!(
            "hotpath-meta: uploaded report {} (repository {}, benchmark {}, baseline {})",
            resp.id,
            resp.repository,
            resp.benchmark,
            resp.baseline.as_deref().unwrap_or("none")
        ),
        Err(reason) => eprintln!("hotpath-meta: upload failed: {reason}"),
    }
}
