...
pub mod tests {
    ...
    fn test_data_endpoints() {
        use hotpath::FormattedFunctionsJson;
        ...
        if let Some(first_function) = timing_response.data.first() {
            use base64::Engine;
            let encoded_name =
                base64::engine::general_purpose::STANDARD.encode(first_function.name.as_bytes());
