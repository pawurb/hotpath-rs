...
Returns JSON array of recent executions with timestamps and durations. Use sql first to get query IDs, then use this tool to get detailed logs."#)]
    ...
    #[tool(
        description = r#"Get execution-time metrics for HTTP requests captured via hotpath's reqwest instrumentation.
...
Requires wrapping the reqwest client with hotpath::http!(...) (or attaching ReqwestHttpMiddleware to a reqwest-middleware stack). Use http_logs with an endpoint id to get recent individual requests."#
    )]
