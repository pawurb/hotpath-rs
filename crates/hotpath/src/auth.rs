//! Shared-secret token check used by the metrics and MCP servers. The token is
//! compared verbatim against the `Authorization` header, without any scheme
//! prefix handling.
...
mod tests {
    ...
    fn token_from_env_filters_empty() {
        ...
        assert_eq!(token_from_env(var), Some("abc".to_string()));
        std::env::remove_var(var);
    }
}
