...
pub(crate) struct SqlState {
    pub(crate) event_tx: CbSender<Vec<SqlEvent>>,
...
/// Initialize the SQL statistics collection system (called on first wrapped pool).
                        ...
                        recv(shutdown_rx) -> _ => {
                            ...
                            if let Ok(mut shared) = inner_clone.write() {
                                ...
                                for event in drained_events {
                                    process_sql_event(&mut shared, event);
                                }
                            }
                            break;
                        }
