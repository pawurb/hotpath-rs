...
//! ```text
//! target = "sqlx::query"
//! fields: db.statement = "<SQL>", summary, elapsed = <Duration>,
//!         elapsed_secs = <f64>, rows_affected, rows_returned
//! ```
//!
...
//!
//! Unlike the old `Executor` pool wrapper, this captures transaction-internal
//! and acquired-connection queries too (logging is at the statement level) and
    ...
    S: Subscriber + for<'a> LookupSpan<'a>,
{
    fn on_event(&self, event: &Event<'_>, _ctx: Context<'_, S>) {
        ...
        // sqlx 0.9 puts the full SQL in `db.statement` only when it differs from
        // the 4-word `summary` (longer queries); for short queries `db.statement`
        // is empty and `summary` holds the whole text. Prefer the full statement,
