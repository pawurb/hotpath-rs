//! Run with:
//!   cargo run -p test-rw-lock-parking-lot --example benchmark_rw_lock_parking_lot --features hotpath
...
fn main() {
    ...
    let (base_w, base_r) = bench!(parking_lot::RwLock::new(0u64));
    let (ins_w, ins_r) = bench!(hotpath::rw_lock!(
        parking_lot::RwLock::new(0u64),
        label = "counter"
    ));

    report("parking_lot RwLock writes", runs, base_w, ins_w);
    report("parking_lot RwLock reads", runs, base_r, ins_r);
}
