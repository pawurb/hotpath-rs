//! Run with:
//!   cargo run -p test-rw-lock-async-lock --example benchmark_rw_lock_async_lock --features hotpath
...
fn main() {
    ...
    future::block_on(async {
        ...
        let (base_w, base_r) = bench!(async_lock::RwLock::new(0u64));
        let (ins_w, ins_r) = bench!(hotpath::rw_lock!(
            async_lock::RwLock::new(0u64),
            label = "counter"
        ));

        report("async-lock RwLock writes", runs, base_w, ins_w);
        report("async-lock RwLock reads", runs, base_r, ins_r);
    });
}
