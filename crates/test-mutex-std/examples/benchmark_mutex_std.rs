//! Run with:
//!   cargo run -p test-mutex-std --example benchmark_mutex_std --features hotpath
...
fn main() {
    ...
    let baseline = bench!(std::sync::Mutex::new(0u64));
    let instrumented = bench!(hotpath::mutex!(
        std::sync::Mutex::new(0u64),
        label = "counter"
    ));

    report("std Mutex", runs, baseline, instrumented);
}
